    ...
    for(let heading of headings) {
      ...
      for(let line of heading.querySelectorAll('.line')) {
        ...
        for(let letter of letters) {
          line.innerHTML += `<span class="letter ${(i > 15 && i < 24 && j === 1) || (i > 6 && i < 20 && j === 2) || (i > 4 && i < 8 && j === 3) ? 'italic' : ''} ${(i > 15 && i < 24 && j === 1) || (i > 20 && i < 26 && j === 3) ? 'beige' : ''} ${letter === ' ' ? 'space' : ''}" style="transition-delay: ${i*15}ms;">${letter}</span>`
          i++
        }
      }
      j++
  ...
  return (
    <>
      ...
      <div className="wrapper">
        ...
        <section id="black" className="h-screen bg-black">
            <Link to="/header" className="w-full h-full inline-block" href="/header"></Link>
        </section>
      </div>
    </>
  );
